...
use tfhe::prelude::*;
use tfhe::{
    generate_keys, set_server_key, ClientKey, Config, ConfigBuilder, FheBool, FheUint32, PublicKey,
    ServerKey,
};
use tqdm::tqdm;
...
pub enum FederatedMessage {
    ...
    RequestComparison((Attribute, FheUint32)),
    ///
...
pub fn encrypt_boolean_private(bool: bool, private_key: &ClientKey) -> FheBool {
    //FheBool::encrypt(value, private_key)
...
pub fn encrypt_boolean(bool: bool, public_key: &PublicKey) -> FheBool {
    //FheBool::encrypt(value, public_key)
    FheBool::encrypt_trivial(bool)
}

pub fn sanitize_activities(max_activities: u32, enc_activity: &Vec<FheUint32>) -> Vec<FheUint32> {
    let mut result: Vec<FheUint32> = Vec::new();
...
pub fn preprocess_trace_private(
    ...
    trace: &Trace,
) -> (Vec<FheUint32>, Vec<FheUint32>) {
    let mut activities: Vec<FheUint32> = Vec::with_capacity(trace.events.len());
    ...
    trace.events.iter().for_each(|event| {
        ...
        let activity_pos: u32 =
            u32::try_from(activity_to_pos.get(&activity).unwrap().clone()).unwrap_or(0);
        activities.push(encrypt_value_private(activity_pos, private_key));
...
pub fn compute_case_to_trace_private(
    ...
    private_key: &ClientKey,
    event_log: &EventLog,
) -> HashMap<Attribute, (Vec<FheUint32>, Vec<FheUint32>)> {
    let mut result: HashMap<Attribute, (Vec<FheUint32>, Vec<FheUint32>)> = HashMap::new();
...
pub fn preprocess_trace(
    ...
    trace.events.iter().for_each(|event| {
        ...
        let activity_pos: u32 =
            u32::try_from(activity_to_pos.get(&activity).unwrap().clone()).unwrap_or(0);
        activities.push(encrypt_value(activity_pos, public_key));
        timestamps.push(encrypt_value(get_timestamp(event), public_key));
    });
...
impl PrivateKeyOrganization {
    ...
    pub fn encrypt_all_data(&self) -> HashMap<Attribute, (Vec<FheUint32>, Vec<FheUint32>)> {
    ...
    pub fn evaluate_secrets_to_dfg<'a>(
        &self,
        secret_edges: Vec<(FheUint32, FheUint32)>,
    ) -> DirectlyFollowsGraph<'a> {
        let mut result = DirectlyFollowsGraph::new();
        let mut found_edges_by_pos: HashMap<(u32, u32), u32> = HashMap::new();
        ...
        for (from, to) in tqdm(secret_edges) {
            let from_pos = self.decrypt(from);
            if from_pos == 0 {
                continue;
            }

            let to_pos = self.decrypt(to);
            if to_pos == 0 {
                continue;
            }
...
impl PublicKeyOrganization {
    ...
    pub fn find_all_conditions(
        &mut self,
        foreign_case_to_trace: HashMap<Attribute, (Vec<FheUint32>, Vec<FheUint32>)>,
    ) -> Vec<(FheUint32, FheUint32)> {
        let own_case_to_trace: HashMap<Attribute, (Vec<FheUint32>, Vec<FheUint32>)> =
        ...
        let max_activities: u32 = u32::try_from(self.activity_to_pos.len()).unwrap_or(0);
        ...
        for case_name in &all_case_names {
            ...
            let (foreign_activities, _) =
                foreign_case_to_trace.get(case_name).unwrap_or(&dummy_tuple);
        ...
        let mut result: Vec<(FheUint32, FheUint32)> = Vec::with_capacity(size_assumption);
        ...
        tqdm(all_case_names.into_iter()).for_each(|case_name| {
            ...
            (foreign_activities, foreign_timestamps) =
                foreign_case_to_trace.get(&case_name).unwrap().to_owned();
            foreign_activities = sanitize_activities(max_activities, &foreign_activities);

            let (own_activities, own_timestamps): (Vec<FheUint32>, Vec<FheUint32>) =
                own_case_to_trace.get(&case_name).unwrap().to_owned();
    ...
    fn find_conditions(
        &mut self,
        foreign_activities: Vec<FheUint32>,
        foreign_timestamps: Vec<FheUint32>,
        own_activities: Vec<FheUint32>,
        own_timestamps: Vec<FheUint32>,
        result: &mut Vec<(FheUint32, FheUint32)>,
    ) {
        ...
        for (i, foreign_timestamp) in foreign_timestamps.iter().enumerate() {
            for (j, own_timestamp) in own_timestamps.iter().enumerate() {
                let foreign_less_equal_own = foreign_timestamp.le(own_timestamp);
        ...
        // Find start
        let start = encrypt_value(1, self.public_key.as_ref().unwrap());
        result.push((
            start,
        ...
        // Find end
        let end = encrypt_value(2, self.public_key.as_ref().unwrap());
        result.push((
            end,
            comparison_foreign_to_own
                .get(&(foreign_timestamps.len() - 1, own_timestamps.len() - 1))
                .unwrap()
                .select(
                    &own_activities[own_timestamps.len() - 1],
                    &foreign_activities[foreign_timestamps.len() - 1],
                ),
        ));
    ...
    fn find_progress_pairs_crossing(
        &mut self,
        activity_list_from: &Vec<FheUint32>,
        activity_list_to: &Vec<FheUint32>,
        ...
        for i in 0..activity_list_from.len() {
            for j in 0..activity_list_to.len() {
                let bottom: FheUint32 = encrypt_value(0, self.public_key.as_ref().unwrap());

                let mut cond = comparison_map.get(&(i, j)).unwrap().clone();
                if j > 0 {
                    cond = cond.bitand(reverse_comparison_map.get(&(j - 1, i)).unwrap());
                }
                if i < activity_list_from.len() - 1 {
                    cond = cond.bitand(reverse_comparison_map.get(&(j, i + 1)).unwrap());
                }

                let from: FheUint32 = cond.select(&activity_list_from[i], &bottom);
                let to: FheUint32 = cond.select(&activity_list_to[j], &bottom);
    ...
    fn find_progress_pairs_non_crossing(
        &mut self,
        activities: &Vec<FheUint32>,
        len_other_activities: usize,
        comparison_map: &HashMap<(usize, usize), FheBool>,
        reverse_comparison_map: &HashMap<(usize, usize), FheBool>,
        result: &mut Vec<(FheUint32, FheUint32)>,
    ) {
        for i in 0..activities.len() - 2 {
            let bottom: FheUint32 = encrypt_value(0, self.public_key.as_ref().unwrap());

            let mut progress: FheBool;
            if len_other_activities != 0 {
                progress = encrypt_boolean(false, self.public_key.as_ref().unwrap());
            } else {
                progress = encrypt_boolean(true, self.public_key.as_ref().unwrap());
            }

            for j in 0..len_other_activities - 2 {
                let local_progress: FheBool = reverse_comparison_map.get(&(j, i)).unwrap().clone()
                    & comparison_map.get(&(i + 1, j + 1)).unwrap().clone();
