...
impl PublicKeyOrganization {
    ...
    pub fn set_public_keys(&mut self, public_key: PublicKey, server_key: ServerKey) {
        ...
        self.start = Some(crate::federated::organization_struct::encrypt_activity(
            1,
            self.public_key.as_ref().unwrap(),
        ));
        self.end = Some(crate::federated::organization_struct::encrypt_activity(
            2,
            self.public_key.as_ref().unwrap(),
        ));
    }
    ...
    pub fn find_all_conditions(
        ...
        let all_case_names_vec = all_case_names.iter().cloned().collect::<Vec<Attribute>>();
        ...
        result.extend(
            all_case_names_vec
                .into_par_iter()
                .tqdm()
                .flat_map_iter(|case_name| {
                    let server_key = self.server_key.clone().unwrap();
                    ...
                    (foreign_activities, foreign_timestamps) = foreign_case_to_trace
                        ...
                        .to_owned();
                    foreign_activities =
                        self.sanitize_activities(max_activities, &foreign_activities);

                    let (own_activities, own_timestamps): (Vec<FheUint8>, Vec<u32>) =
                        own_case_to_trace
                            .get(&case_name)
                            .unwrap_or(&(Vec::new(), Vec::new()))
                            .to_owned();
    ...
    fn find_conditions(
        ...
        let mut result: Vec<(FheUint8, FheUint8)> = Vec::new();
