...
fn compute_needleman_wunsch_at_pos(
    ...
    let zero = 0.0;
    let val_up = cmp::max_by(values[[pos.0, pos.1 - 1]] - 1.0, zero, |x, y| {
        x.partial_cmp(y).unwrap()
    });
    let val_left = cmp::max_by(values[[pos.0 - 1, pos.1]] - 1.0, zero, |x, y| {
        x.partial_cmp(y).unwrap()
    });
...
mod tests {
    ...
    fn test() {
        ...
        let (values, directions, best_alignment) = compute_needleman_wunsch(&list_4, &list_5);
